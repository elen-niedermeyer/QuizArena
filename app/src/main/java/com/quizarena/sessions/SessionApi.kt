...
class SessionApi {
    ...
    fun requestAllSessions(): List<QuizSession> {
        ...
        val now = Date()
        sessions.add(QuizSession("Test1", Date(now.time + (4 * 60 * 60 * 1000)), "default", true, true))
        ...
        sessions.add(QuizSession("Test4", Date(now.time + (2 * 60 * 60 * 1000)), "default", true, true))
